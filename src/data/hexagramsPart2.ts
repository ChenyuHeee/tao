...
const hengHex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(5, false, '振恒，凶。', '不断地变更破坏长久之道，需要格外谨慎。提示在应该坚守的时候反复无常是不好的。'),
  ];
  return {
    kingWenNumber: 32,
    binary: bin,
...
const daZhuangHex = ((): HexagramData => {
  ...
  const bin = (upper.binary << 3) | lower.binary;
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    line(0, true, '壮于趾，征凶，有孚。', '力量只到了脚趾就想出征，需要格外谨慎。提示力量还不够时不要急于行动。'),
...
const yi2Hex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(2, false, '益之用凶事，无咎。有孚中行，告公用圭。', '在可能存在风险的事情中得到增益和帮助，没有过失。心怀诚信走中道，用圭玉告知王公。'),
    ...
    line(5, true, '莫益之，或击之。立心勿恒，凶。', '没有人来帮他，反而有人来攻击他。如果立下的志向不能持久，就会比较困难。'),
  ];
  return {
    kingWenNumber: 42,
    binary: bin,
...
const guaiHex = ((): HexagramData => {
  ...
  const bin = (upper.binary << 3) | lower.binary;
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    line(0, true, '壮于前趾，往不胜为咎。', '力量只在前面的脚趾上就想前进，去了很可能不胜，这会成为过失。'),
    line(1, true, '惕号，莫夜有戎，勿恤。', '警惕地发出号令，夜晚虽然有敌人来犯，也不必过分担忧。因有备而无患。'),
    line(2, true, '壮于頄，有凶。君子夬夬，独行遇雨，若濡有愠，无咎。', '愤怒表现在脸上，需要格外谨慎。有德之人决断地独行，途中遇雨被淋湿了，虽然有些不快但无大碍。'),
    line(3, true, '臀无肤，其行次且。牵羊悔亡，闻言不信。', '臀部受伤坐立不安，走路踉踉跄跄。牵着羊走悔恨就会消失，但这话说出来没人相信。'),
    line(4, true, '苋陆夬夬，中行无咎。', '像苋陆草那样决断干脆，走中道就不会有过失。'),
    line(5, false, '无号，终有凶。', '没有发出号令和警示，最终可能会遇到麻烦。提示当断不断反受其乱。'),
  ];
  return {
    kingWenNumber: 43,
    binary: bin,
...
const gouHex = ((): HexagramData => {
  ...
  const bin = (upper.binary << 3) | lower.binary;
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    line(0, false, '系于金柅，贞吉。有攸往，见凶。羸豕孚蹢躅。', '系在金属的刹车杆上（加以约束），持守正道则吉祥。若贸然前往则可能存在风险。像瘦猪一样蠢蠢欲动。'),
    line(1, true, '包有鱼，无咎。不利宾。', '包裹里有鱼，没有过失。但不宜用来招待客人。提示先管好自己。'),
    line(2, true, '臀无肤，其行次且。厉，无大咎。', '臀部受伤坐立不安，行走困难。有风险但没有大过错。'),
    line(3, true, '包无鱼，起凶。', '包裹里没有鱼，有所行动则可能存在风险。提示资源不足时不可贸然行动。'),
...
const kun2Hex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(2, false, '困于石，据于蒺藜。入于其宫，不见其妻，凶。', '被岩石困住了，手又扶在了蒺藜上。回到家中却见不到妻子，处境会比较艰难。提示走到了极端无助的境地。'),
...
const geHex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(2, true, '征凶，贞厉。革言三就，有孚。', '贸然前进可能存在风险，即使持守正道也艰难。变革的主张要经过多次讨论才能达成一致，才能获得信任。'),
...
const dingHex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(3, true, '鼎折足，覆公餗，其形渥，凶。', '鼎的脚折断了，把王公的美食打翻在地，弄得一塌糊涂，需要格外谨慎。提示不堪重任。'),
...
const zhenHex = ((): HexagramData => {
  ...
  const bin = (upper.binary << 3) | lower.binary;
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    line(0, true, '震来虩虩，后笑言哑哑，吉。', '雷声滚滚而来令人不安，过后仍然谈笑自若，吉祥。'),
    line(1, false, '震来厉。亿丧贝，跻于九陵。勿逐，七日得。', '雷声来得猛烈。估计会失去财宝，于是登上高陵。不要去追逐，七天之后自然会失而复得。'),
    line(2, false, '震苏苏，震行无眚。', '在雷声中感到不安，但若以敬畏之心来行事则不会有问题。'),
    line(3, true, '震遂泥。', '雷声过后陷入了泥沼。提示被不安所压垮。'),
    line(4, false, '震往来厉。亿无丧，有事。', '雷声来来回回都很猛烈。但估计不会有什么大的损失，只是需要处理一些事情。'),
    line(5, false, '震索索，视矍矍，征凶。震不于其躬，于其邻，无咎。婚媾有言。', '被雷声吓得哆嗦发抖，眼神中透着不安。此时出行可能存在风险。雷震没有伤到自己，而伤了邻居，没有大碍。但在婚姻嫁娶之事上会有一些口角和争议。'),
  ];
  return {
    kingWenNumber: 51,
    binary: bin,
    ...
    guaCiTranslation: '通达。雷声滚滚而来令人不安，但镇定下来后依然谈笑自若。雷声震惊百里，但主持祭祀的人手中的勺子和香酒并没有掉落。震卦象征震动与敬畏。',
    upperTrigram: upper,
    lowerTrigram: lower,
    lines,
  };
...
const jian2Hex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(2, true, '鸿渐于陆，夫征不复，妇孕不育，凶。利御寇。', '大雁渐渐飞到陆地上，丈夫出征未归，妇人怀孕却不生育，处境会比较艰难。此时只适宜用来防御外敌。'),
...
const guiMeiHex = ((): HexagramData => {
  ...
  return {
    ...
    pinyin: 'guī mèi',
    guaCi: '征凶，无攸利。',
    guaCiTranslation: '贸然前进可能存在风险，没有什么有利之处。归妹卦象征婚姻与结合，但也提示在不对等的条件下结合是有风险的。',
    upperTrigram: upper,
    lowerTrigram: lower,
    lines,
  };
...
const fengHex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(5, false, '丰其屋，蔀其家，窥其户，阒其无人，三岁不觌，凶。', '丰盛了他的房屋，却遮蔽了他的家。从门缝中窥视，寂静无人，多年不见人影，提示需要格外审慎。提示盛极而衰、与人疏远。'),
  ];
  return {
    kingWenNumber: 55,
    binary: bin,
...
const lv2Hex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(5, true, '鸟焚其巢，旅人先笑后号咷。丧牛于易，凶。', '鸟的巢被烧了，旅途中的人先笑而后大哭。在边界丢失了牛，处境会比较艰难。提示在旅途的终点放松警惕会招致损失。'),
  ];
  return {
    kingWenNumber: 56,
    binary: bin,
...
const xunHex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(5, true, '巽在床下，丧其资斧，贞凶。', '过分地跪伏在床下，连生活所需的资财和工具都失去了。即使持守正道也可能存在风险。提示顺应过度就会失去自我。'),
  ];
  return {
    kingWenNumber: 57,
    binary: bin,
...
const xiaoGuoHex = ((): HexagramData => {
  ...
  const bin = (upper.binary << 3) | lower.binary;
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    line(0, false, '飞鸟以凶。', '像飞鸟一样向上冲是可能存在风险的。提示不要好高骛远。'),
    line(1, false, '过其祖，遇其妣。不及其君，遇其臣。无咎。', '越过了祖父却遇到了祖母。没有见到君王而遇到了臣子。没有过失。提示退而求其次也能接受。'),
    line(2, true, '弗过防之，从或戕之，凶。', '不去过度防范，但若放任自流可能会被伤害，需要谨慎对待。提示要适度保护自己。'),
    ...
    line(5, false, '弗遇过之，飞鸟离之，凶。是谓灾眚。', '没有遇到反而远远地超过了，像飞鸟离开了该去的地方，提示需要格外审慎。这就是意想不到的困难。'),
  ];
  return {
    kingWenNumber: 62,
    binary: bin,
...
const weiJiHex = ((): HexagramData => {
  ...
  const lines: [LineData, LineData, LineData, LineData, LineData, LineData] = [
    ...
    line(2, false, '未济，征凶。利涉大川。', '事情还没有成功，贸然前进可能存在风险。但适宜渡过大的河流。提示条件具备时就要大胆去做。'),
